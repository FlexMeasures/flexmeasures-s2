...
from ..models.device_data import FRBCDeviceData
...
class ConnectionState:
    ...
    def __init__(self):
        ...
        self.sent_instructions: List[
            FRBCInstruction
        ] = []  # Store sent instructions for revocation
        self.instruction_statuses: Dict[
            uuid.UUID, InstructionStatus
...
class S2FlaskWSServerSync:
    ...
    def __init__(
        ...
        self._websocket_to_resource: Dict[
            Sock, str
        ] = {}  # Map websocket to resource_id
        self._connection_states: Dict[
            Sock, ConnectionState
        ] = {}  # Track connection state for rate limiting
        self._register_default_handlers()
        self.sock.route(self.ws_path, bp=flexmeasures_s2_api_bp)(self._ws_handler)
        ...
        # Make discoverable
        self.blueprint = flexmeasures_s2_api_bp
        flexmeasures_s2_api_bp.record(self._on_registered)

    def _on_registered(self, state):
        app = state.app  # ← this is the real Flask app instance
        app.extensions["s2_ws_server"] = self
