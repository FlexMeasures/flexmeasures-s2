...
class CongestionPointPlanner:
    ...
    def create_initial_planning(self, plan_due_by_date: datetime) -> JouleProfile:  # noqa: C901
        ...
        if self.congestion_target.is_within_range(current_planning):
            print("Current planning is within the congestion target range. Returning it.")
            return current_planning

        # If the current planning is not within the congestion target range, optimize it
        print("Current planning is not within the congestion target range. Optimizing it.")
                ...
                for device in self.devices:
                    if device.priority_class <= priority_class:
                        try:
                            ...
                            if proposal is None:
                                raise ValueError(f"No proposal found for device {device.device_id}")
                            print(
