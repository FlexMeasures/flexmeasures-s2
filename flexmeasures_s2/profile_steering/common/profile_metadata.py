...
class ProfileMetadata:
    ...
    def get_profile_start_at_timestep(self, i: int) -> datetime:
        if i >= self.nr_of_timesteps or i < 0:
            raise ValueError(
                f"Expected i to be between 0 <= i < {self.nr_of_timesteps} but was {i}"
            )
    ...
    def to_dict(self) -> dict:
        return {
            ...
            self.TIMESTEP_DURATION_KEY: int(
                self.timestep_duration.total_seconds() * 1000
            ),
    ...
    def from_dict(data: dict) -> "ProfileMetadata":
        profile_start = datetime.fromtimestamp(
            data[ProfileMetadata.PROFILE_START_KEY] / 1000
        )
        timestep_duration = timedelta(
            milliseconds=data[ProfileMetadata.TIMESTEP_DURATION_KEY]
        )
    ...
    def subprofile(self, new_start_date: datetime) -> "ProfileMetadata":
        if new_start_date < self.profile_start:
            raise ValueError(
                "The new start date should be after the current start date"
            )
        new_start_date = self.next_aligned_date(new_start_date, self.timestep_duration)
        skipped_steps = int(
            (new_start_date - self.profile_start).total_seconds()
            // self.timestep_duration.total_seconds()
        )
    ...
    def adjust_nr_of_elements(self, nr_of_elements: int) -> "ProfileMetadata":
        return ProfileMetadata(
            self.profile_start, self.timestep_duration, nr_of_elements
        )
    ...
    def next_aligned_date(date: datetime, timestep_duration: timedelta) -> datetime:
        ...
        remainder = (
            date - datetime.min
        ).total_seconds() % timestep_duration.total_seconds()
        if remainder == 0:
            return date
