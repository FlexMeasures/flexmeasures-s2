...
class RootPlanner:
    ...
    def __init__(self, target: Any, energy_iteration_criterion: float, cost_iteration_criterion: float, context: Any):
        """
    ...
    def plan(self, plan_due_by_date: datetime, optimize_for_target: bool, max_priority_class_external: int, multithreaded: bool = False):
        ...
        for priority_class in range(min_priority_class, min(max_priority_class, max_priority_class_external) + 1):
            ...
            while True:
                ...
                i += 1
                print(f"Root controller: selected best controller '{best_proposal.get_origin().get_device_name()}' with global energy impr {best_proposal.get_global_improvement_value()}, cost impr {best_proposal.get_cost_improvement_value()}, congestion impr {best_proposal.get_congestion_improvement_value()}, iteration {i}.")
                
                # Check stopping criteria: if improvement values are below thresholds or max iterations reached.
                if ((best_proposal.get_global_improvement_value() <= self.energy_iteration_criterion and
                     best_proposal.get_cost_improvement_value() <= self.cost_iteration_criterion) 
                     or i >= self.MAX_ITERATIONS):
                    break
            
            print(f"Optimizing priority class {priority_class} was done after {i} iterations.")
            if i >= self.MAX_ITERATIONS:
                print(f"Warning: Optimization stopped due to iteration limit. Priority class: {priority_class}, Iterations: {i}") 
