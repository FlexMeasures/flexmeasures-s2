...
from flexmeasures_s2.profile_steering.frbc.s2_frbc_device_planner import S2FrbcDevicePlanner
...
@pytest.fixture
def setup_planner() -> tuple[S2FrbcDevicePlanner, datetime, ProfileMetadata]:
    ...
    profile_metadata = ProfileMetadata(
        ...
        nr_of_timesteps=288
    )  # Mock or create a real instance
    plan_due_by_date = profile_metadata.get_profile_start().plus_seconds(10)
    ...
    expected_joule_profile = JouleProfile(profile_start=profile_metadata.get_profile_start(), timestep_duration=profile_metadata.get_timestep_duration(), elements=JouleProfileTarget)
    assert joule_profile == expected_joule_profile

def test_create_initial_planning(setup_planner):
    planner, plan_due_by_date, profile_metadata = setup_planner
